...
static int num_threads = 8; // pthreads
static int num_iter = 25; // Number of full passes through cooccurrence matrix
static int vector_size = 50; // Word vector size
static int save_gradsq = 0; // By default don't save squared gradient values
static int use_binary = 1; // 0: save as text files; 1: save as binary; 2: both. For binary, save both word and context word vectors.
static int model = 2; // For text file output only. 0: concatenate word and context vectors (and biases) i.e. save everything; 1: Just save word vectors (no bias); 2: Save (word + context word) vectors (no biases)
static real eta = 0.05; // Initial learning rate
static real alpha = 0.75, x_max = 100.0; // Weighting function parameters, not extremely sensitive to corpus, though may need adjustment for very small or very large corpora
